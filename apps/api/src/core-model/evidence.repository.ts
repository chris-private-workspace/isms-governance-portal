 ...
 *
 *   **`linkedType` is not an input.** EvidenceLinkedType has one value, so the
 *   only thing a caller could legally pass is the one this file sets. A field with
 *   a single legal value is not a field — the same reasoning ControlRepository
 *   applies to `appliesToScope`. When attestation or assessment arrive, this
 *   becomes an input in the same change that gives the trigger its second branch,
 *   and not before.
 *
 * Key Components:
...
export interface CreateEvidenceInput {
  ...
  /**
   * The control test this evidences. Whether it is reachable is decided by the
    ...
    try {
      ...
      return await client.evidence.create({
        data: {
          ...
          // The one legal value, set here rather than accepted — see the header.

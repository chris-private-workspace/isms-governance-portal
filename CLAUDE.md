...
| **Current Phase** | **W08**（PR #47 pending）`closed` — M1 slice 5；`Issue` + `Action`（CAPA）+ 兩組端點。W07 的 D1 判準**第一次導出另一個答案**：`issues` 給得起複合錨點，所以走 **FK 而非 trigger**（N1 移掉它 → 跨實體引用插入成功，3 紅 8 綠）。**14/36 實體**（`check_entity_index.py` 機械導出，`run_all` 7/7；其餘 22 是 slice 6..N）|

...
| **Current Phase** | **W07 MERGED**（PR #44, `19bc4f7`）`closed` — M1 slice 4；`ControlTest` + `Evidence` + 兩組端點。父表拒絕複合錨點時，跨實體引用由 **`BEFORE INSERT OR UPDATE` + `SECURITY INVOKER` trigger** 擋（**RI 檢查繞過 RLS** 是量出來的）。**12/36 實體**（W08 起由 `check_entity_index.py` 導出，`run_all` 7/7；其餘 24 是 slice 5..N）|
...
**Last Updated**: 2026-08-12（W07 closeout）

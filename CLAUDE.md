...
| **Current Phase** | **W21**（PR #84 開著）`closed_partial` —— web demo 上了 Azure，29 路由對**真實網址**走查通過；M0 DoD #5 **仍是部分**（憑證是平台預設）、#3 首次有標的且已量測。⛔ CI 自動部署未做（GitHub Actions 無 Azure 身分）。資源與重建：[`infra/azure/README.md`](./infra/azure/README.md) |

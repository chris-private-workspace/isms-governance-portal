...
- **Project Alignment**: Read `CLAUDE.md` §Core Vision + §Architecture and align every design decision with them. Respect the project's declared scope boundaries (`docs/rules-on-demand/scope-boundaries.md`) — a design that requires a cross-scope violation needs to be reworked, not waived. Check `docs/14-adr/` before proposing anything that reverses a settled decision.

...
- W19 [Mockup port —— 全綠可以是假的零](memory/project_w19_mockup_port.md) — PR-pending，`closed`。設計交付物 **30 / 30 個畫面**落地；⭐⭐ **三層驗證各自抓到對方看不見的東西** —— gate 綠、保真度比對 719 字串漏抄 0，而 **drive-through 抓到 25 個死控件**，它們通過了每一項 gate 含本片自己新加的 hover 守衛。Keywords: dead controls, data-hov disabled, drive-through, posture threshold drift, agent parallelism factor, AD-LocalPasswordFallback-1

...
- W17 [保存政策與法務扣留](memory/project_w17_retention_and_legal_hold.md) — PR-pending 2026-08-16, **`closed`**; **32/36**，一張全域表 + 一張 entity-scoped，零端點。⭐⭐ 最重要的產出是**不建**多型守衛 —— W14 那支 `::uuid` cast 早於 mapping walk，`class` 不是 uuid。⭐ 中性化 **N4 零轉紅**揭露真缺口。

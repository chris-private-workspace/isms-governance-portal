...
- W14 [Attestation + 第二個多型連結](memory/project_w14_attestation.md) — PR-pending 2026-08-15, **`closed_partial`**; **22/35**。⭐ W13 漂移守衛**第一次實戰**：建表而未接稽核 ⇒ 恰好 1 紅、訊息自己指名。⭐⭐ Day 0 的 D5 擋下恆真測試 —— group-shared control 跨實體**合法**可讀。⛔ N3b 預測錯 ⇒ enum **三份真相**，`ALTER TYPE ADD VALUE` 必配 `prisma generate`。

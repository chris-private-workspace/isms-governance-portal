...
- W13 [Audit coverage 1 → 15](memory/project_w13_audit_coverage.md) — PR #61 pending 2026-08-15, **`closed`**; **15/21 覆蓋**（機械導出）。⭐⭐ **plan 的做法在寫第一條測試前被實驗推翻**：module-local 圖裡稽核是關的（`before=9 after=9`）⇒ 原做法只能永遠紅或寫成 `>= 0`（**正是本片要消滅的 Potemkin**）。⭐ **N2 恰好 2 紅**，其餘 14 條未動 ⇒ 覆蓋逐模型成立。

...
- W09 [Shared assessment engine](memory/project_w09_assessment_engine.md) — PR #50 pending 2026-08-13, **`closed`**; **17/35**（分母 −1：`Assessment` 判為用例不是表）。⭐ `template_version` 逼出三選一，選了「DB 在 BEFORE INSERT 填」，而該 trigger 用 `COALESCE` 不 `RAISE` 才沒自己開 oracle。元驗證 **6/6 方向全中**；⛔ 第一次 N1 改資料庫被重建覆蓋，**20/20 全綠而什麼都沒量到**。

...
- W18 [Event 骨架與 posture 快照](memory/project_w18_event_and_posture.md) — PR-pending 2026-08-17, **`closed`**; **34/36**，兩張 entity-scoped append-only 表、零端點。⭐⭐ 核心是一個**區分**：構造相同而理由不可互換 —— 快照是規格明文且**無解封**，事件是能力尚不可表達且 M6 解封。⭐ Day-0 **D1 推翻 plan 自己的理由**（論證漂移，沒有 gate 抓得到）。

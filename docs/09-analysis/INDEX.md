...
| 日期 | 文件 | 一句話 |
|------|------|--------|
| 2026-08-19 | [`fixture-prose-inventory-20260819.md`](./fixture-prose-inventory-20260819.md) | 27 個畫面上對「某一筆記錄」做出的治理陳述，**逐頁**清單（約 180 條）—— 接 API 前要清的帳。⭐ 槓桿最高的一條**不在任何 `page.tsx` 裡**（`AppShell.tsx:448`，25 頁）；`/controls/[id]` **仍在做 `/risks/[id]` 已被明令禁止的事**。含 affordance 的**收斂規則**（沒有它重跑會得到不同數字）與覆蓋聲明 |

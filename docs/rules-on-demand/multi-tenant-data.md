...
**中國在範圍內，PIPL 要求境內處理與儲存**（已確認參數 #4、guardrail 8、ADR-0006）。
但滾升儀表板要跨區聚合。這兩者直接衝突，**且尚未有定案** ——

> **ADR-0006 未拍板前，不要實作跨區滾升。**
> 可能的方向（各有代價，由 ADR 決定）：只跨區傳送聚合值不傳原始記錄、
> 中國區獨立儀表板不參與集團滾升、或以脫敏指標同步。
> **這是 M0 阻斷項。碰到就停下來問，不要自己選一個。**

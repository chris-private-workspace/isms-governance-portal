...
| **字面 actual** | **~15.3 hr** → ratio **2.19**（OVER band 兩倍以上）|
| 其中**跨夜間隙**（base → 本 phase 第一個 commit `785be55` 09:31:41）| **10h20m** |
| **扣除間隙後** | **~5.0 hr** → ratio **0.71**（IN band，貼著下緣）|
...
⚠️ **修正值 ~5.0 hr 是下界**：plan 與 checklist 的起草發生在第一個 commit **之前**，
...
- [x] `|R - 1.0|` 兩種讀法分別為 **119%** 與 **29%** → **記 AD**（`AD-CalibrationMetric-2`）

...
**Created**: 2026-08-07
**Last Reviewed**: 2026-08-12（W07 closeout —— R4 更新，其餘七條逐條看過無變化）
**Status**: Active
...
| R4 | ⭐ **稽核軌跡尚不存在，而資料已經在寫** —— guardrail 5 要求所有狀態變更進 append-only 防篡改日誌；**沒有任何稽核軌跡**。滾升是合法的跨實體讀取，它**必須被稽核**才不會變成後門。⚠️ **敞口逐 phase 擴大**：W02 兩張表 → W04 +2（`users` / `ref_code_counters`）→ W05 +3（`asset_groups` / `assets` / `risks`）→ W06 +1（`controls`）→ **W07 +2**（`control_tests` / `evidence`），今日**十張表、無一有稽核**。⛔ **W06 的擴大不只是 +1 張表**：`asset_groups` / `assets` 在 W05 沒有寫入路徑，它們的無稽核寫入當時只是理論上的；W06 給了它們端點，**三條無稽核寫入路徑同時變成真的可達** | **確定（現況）** | 高 | 攔截點已就位（ADR-0004 的 client extension 就是 M3 的落點）。⚠️ **緩解措施本身沒有進展** —— 五個 phase 以來擴大的是敞口不是防護；`created_by`/`updated_by` 欄位存在但**永遠是 NULL**（刻意：填佔位使用者會讓 M3 的問題用謊話被回答）。⭐ **W06 新增一個 M3 必須答的問題**：group-shared control 的**跨實體讀取**是合法的滾升路徑，依 guardrail 4「滾升路徑自身也要被稽核」，它需要的不只是寫入稽核。⭐⭐ **W07 讓 R4 從「敞口」變成「反諷」**：`evidence` 這張表**存在的目的就是證據等級的主張**（`hash` 是完整性錨點），而**寫入它的動作本身沒有任何稽核軌跡** —— 一筆沒有人知道是誰、何時、憑什麼建立的證據記錄。⚠️ `hash` 目前**也沒有任何東西驗證它**（無演算法約定、無重算比對）→ M3 必須同時解這兩半 | **開放** —— M3 / ADR-0003 / OQ-4。⛔ **每個新增業務表的 phase 都會讓它更大，而沒有任何 gate 會叫** | 2026-08-12 |

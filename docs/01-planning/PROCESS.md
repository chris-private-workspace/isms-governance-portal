...
incoming task
    ...
    │
    ├─► 純稽核 / 分析 —— 不改任何東西，產出是報告？        → 不走軌
    │                                                        寫 docs/09-analysis/ + commit
    │
    ├─► 產出就是一份 ADR？                                 → 不走軌
    │                                                        直接寫 docs/14-adr/NNNN-*.md
    │
    └─► trivial（typo / 單行 / < 30 分鐘）？               → 直接 commit，免文件
```

> **「既定設計」的界線 ——** 已 approve 且**約束後續工作**的東西才算：
> `docs/02-architecture/` 的設計文件、**已採納**的 ADR、`02a` 的資料模型規格。
> 起草中、未拍板的東西**不算**。
>
> 沒有這道限制，Change 軌會變成什麼都能塞的垃圾桶。
> **真實案例**：`CH-001`～`008` 期間本判準原本寫「改現有**功能的**行為」，
> 而本專案當時沒有 code —— 於是 8 個 CH 裡有 6 個是硬套進來的（`CH-009` §Problem）。
>
> **兩條「不走軌」不是偷懶，是避免空殼**：一份稽核報告的價值在報告本身，
> 一個 ADR 的價值在 ADR 本身；再包一層 spec + checklist + progress
> 只是把同一段歷史寫兩遍（`CH-005` 就是這樣，整個交付物就是「寫三份 ADR」）。
> ⚠️ 但**稽核如果導致要改東西**，那個「改」是 Change 軌 —— 分析與變更分開走。
...
| 「**改掉 / 收窄 / 擴大範圍**」/「**這個設計要改**」/「**某某不做了**」 | Change（改既定設計）|
| 「X 不 work」/「壞了」/「fail」/「regression」/「錯了」 | Bug-fix |
| 「**查一下 X 跟規格對不對**」/「**盤點**」/「**audit**」 | 不走軌 → `09-analysis/` 報告 |
| 「**決定用 A 還是 B**」/「**把這個決定寫下來**」 | 不走軌 → 直接寫 ADR |
...
| 用 Change | **不要**用（改走別軌）|
|---|---|
| 改現有**行為**，或改**既定設計**（見 §1.1 的界線）| 全新**產品**功能 → Phase |
| **新增內部工具 / lint / CI 步驟**（非產品功能）且 < 3 天 | 跨週的工具建置 → Phase |
| Scope < 3 天 | 跨週 → Phase |
| 行為本來是對的，現在要改 | 行為**本來對、現在錯了** → Bug-fix |
| **行為一直都錯 / spec 有缺口** —— 那不是 regression，是缺口 | 純稽核／分析、純 ADR → 不走軌（§1.1）|
| **重構**（行為不變，改結構）| 使用者看得到差異 → 那是行為變更，仍是 Change 但要標明 |

> ⚠️ **第 3、4 列曾經自相矛盾。** 舊版 §3.1 寫「行為本來就錯 → Bug-fix」，
> 而 §4.1 寫「行為一直都錯 / spec 有缺口 → Change」—— 同一個輸入兩個相反路由，
> 答案取決於讀者剛好翻到哪一節（`CH-009` §Problem 發現）。
>
> **以 §4.1 為準**，理由不是「哪個先寫」：Bug 軌的核心價值是**「為什麼測試沒抓到」**，
> 而一個從來沒對過的行為沒有「沒抓到」可言 —— 它是 spec 缺口，該問的是「為什麼當初沒規定」。
> `CH-006`（CI 從第一個 PR 起就沒綠過）走 Change 是對的。

> **第 2 列為什麼存在**：`CH-007`（新增一個 lint detector，1 天）在舊判準下只能落到
> 「全新功能 → Phase」，但為它開一個 phase 四件套（plan / checklist / progress / retrospective
> \+ workload calibration）純粹是儀式。**產品功能與內部工具的成本結構不同**，判準要分開。
> 界線是「使用者會不會看到它」—— 看不到的是工具。
...
| 用 Bug-fix | **不要**用 |
|---|---|
| 行為本來對（per spec）但現在錯了 | 行為一直都錯 / spec 有缺口 → Change（§3.1 已對齊）|
| Fix < 3 天 | 跨週重構 → Phase |

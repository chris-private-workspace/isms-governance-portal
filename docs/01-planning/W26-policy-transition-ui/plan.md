...
| **R12 —（Day-0 D4 裁決）`shell.inert` 走「最小修」** —— 使用者 2026-08-21 選定：**只給 `/policies` 的 `New policy` 一個自己的 key**，不動其餘 23 個 call site | ⇒ plan §3.x 的「❌ 拆 `shell.inert` key」**部分推翻**，而推翻的理由是**本片製造了那個矛盾**（你只修自己弄壞的那一處）。存量 23 處仍屬 `AD-SharedInertProseInaccurate-1`。⚠️ 新 key 要**兩個 locale 同時加**，否則 parity test 紅 |

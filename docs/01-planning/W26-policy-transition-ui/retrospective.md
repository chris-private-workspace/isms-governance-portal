...
| `AD-SelfRolledSettleCheck-1` | ⭐ **自製的「CI 跑完了嗎」迴圈把「零個 check」讀成「全部通過」** —— 條件寫的是「輸出裡沒有 `pending` 就算 settle」，而剛 push 後 check 尚未註冊時 `gh` 回的是 **`no checks reported`** —— 裡面沒有 `pending`，於是迴圈宣告 settle 並退出，exit 0 | ⛔ **同一天第 3 次同形**：(1) 用窄 pattern 過濾掉失敗輸出 · (2) 用 diff 空不空證明刪除安全 · (3) 本條。共通點是**自製一個代理判斷去回答一個有專用工具的問題**。⇒ 改用 `gh pr checks --watch`（它自己知道什麼叫 settle）。⚠️ Monitor 工具說明**逐字寫著這件事**（*silence is not success* / 「若這個 process 現在崩潰，我的 filter 會吐出東西嗎」）—— **規則讀過了仍然犯**，所以這條的價值不在「知道」而在下次動手前**先問有沒有專用工具** | 候選 |

...
**PR**: #61（**MERGED** 2026-08-15，`91bd789`）
...
| US-4 | 四個中性化，預期方向先 commit，結果逐項對照 | ✅ 完成 —— `54a509a` 先行；2 個完全命中、2 個方向對但數字少算 |
...
- **量法自報**（`AD-CalibrationDay0InOrOut-1`）：**含 Day 0**，窗口 =
  branch 第一個 commit（`b469623`，22:48:06）→ **closeout commit**（`25ce0e6`，12:28:27）
- **Ratio**: **0.884 – 0.921**
- **Band 判定**: **IN**（0.7–1.2），⭐ **區間兩端同 band ⇒ 判定不依賴推估的那 5 分鐘**

⛔ **而那個自報的窗口這次給出 13.67 hr / ratio 6.08** —— 一眼可見的荒謬值。
`73f4d9b` → `25ce0e6` 之間有 **729 分鐘**的間隙，因為 phase 跨夜、Day 4 是使用者隔天啟動的。

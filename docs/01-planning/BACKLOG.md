...
| AD-UserEntitySpec-1 | ⭐ **`User` / `Role` / `Permission` 沒有任何欄位級規格，而 `02a` §0 自稱 "Every entity in the platform" 且明訂「不在清單上的不可建」** —— 三處互相指向、無一處定義：`02a:92`（base field `owner_user_id UUID FK → User`，是**引用**）· `02:37`（一列概念，指向 `05`）· `05` §Identity（描述 OIDC 委派與授權模型，**無欄位**）| 完成距離分析 2026-08-10 | 🟡 P1 | ⚠️ **越晚做越貴，且成本是超線性的**：照 `02a` §1.1，每個 domain entity 都有 `owner_user_id` / `created_by` / `updated_by` 三個 FK 指向 `User`。M1 建 32 張表時若 `User` 仍無規格，**M4 要回頭替 32 張表各加 3 欄 + migration + 回填**。✅ **已拍板 2026-08-10（使用者）：先建最小 `User`，作為 M1 的第一個 deliverable** —— 先於其餘 32 張表，因為每張表的 3 個 FK 都指向它。⛔ **拍板的是排序不是欄位** —— 欄位集合待 M1 plan 定義（來源：`05` §Identity 的授權模型 + `02a` §1.1 的 FK 需求），**不得由助手自行發明**。⚠️ 依 `02a` §0「adding an entity means adding a row here in the same change」，M1 的 change 必須**同時**把 `User`（及 `Role` / `Permission` 若一併建）加進索引表 —— 否則就是在違反自己剛引用的那條規則。與 `AD-Model-Gaps` 不同：那條是**已規格實體的欄位缺口**，這條是**實體本身沒有規格** |

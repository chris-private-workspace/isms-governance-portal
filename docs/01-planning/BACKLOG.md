...
> （**現為 65 條 —— P0 5 / P1 40 / P2 20**，2026-08-11 W06 Day 1 加入 `AD-GroupRowTheft-1`；
> 逐列重數過，不是把 64 加一）。
...
| AD-ClaudeMdBudget-1 | **`CLAUDE.md` 長期貼著 byte 上限運行** —— CH-009 當時 28,968 / 30,000（96.6%），headroom 1,032。⛔ **2026-08-11（W05 closeout）實測 29,785 / 30,000 —— headroom 剩 215（99.3%）**，且本次 closeout 已是「換掉一行 + 加兩個 ADR 編號」的最小改動 | CH-009 | 🟡 **P1**（**觸發條件已成立**）| 觸發條件（headroom < 500）**已達成**：下一次實質新增就會撞牆，而下一個 phase 的 closeout 至少要改 Current Phase 一行。⛔ **做結構性瘦身**（把非導航內容移進 `docs/`），不要再靠壓縮形容詞 —— 那已經做過一輪。⚠️ 這是 CI 會 fail 的硬 gate（`check_rules_hygiene.py`），不是建議 |
...
| AD-GroupRowTheft-1 | **不對稱 RLS 擋不住「奪取」，`DELETE` 更是完全沒有寫入側防護** —— W06 Day 1 在 throwaway DB 實測（`W06-*/artifacts/d1-rls-probe.out`）：對 `extension_fields` 的 policy 逐字複製版，HK1 可以把 `org_entity_id IS NULL` 的 group 列**改成自己持有**（`USING` 放行因為它還是 NULL，`WITH CHECK` 放行因為新持有者是我）→ 該列從其他 12 家眼前消失；`DELETE` 只看 `USING`（**沒有 `WITH CHECK`**）所以跨實體刪除同樣成功 | W06 Day 1 | 🟡 P1 | ⚠️ **今天不可透過應用程式觸發**：`scoped-client.types.ts:96-100` 對 `extensionField` 只暴露 `findMany`，沒有寫入路徑。但 SQL 層的 `GRANT SELECT, INSERT, UPDATE` 與 policy **就是那道控制**，而它有洞。**`DELETE` 今天沒爆是因為沒授 `DELETE`，不是因為 RLS** —— 誰加了 `GRANT DELETE`，policy 一個字不用改洞就開了。修法（W06 實測可行）：拆成 per-command policy，寫入側 `USING` 收窄。⛔ **依節流閘不當場修** |

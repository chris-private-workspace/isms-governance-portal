...
> [`STATUS_AUDIT.md`](./STATUS_AUDIT.md) §2.7）。**尚未修正。**
...
| AD-CacheControl-1 | ~~**`apps/api` 完全沒有設 `Cache-Control`**，而 `16:22` 要求「敏感頁面與 **API 回應**使用 `no-store, private`」~~ → ✅ **W03 CLOSED** | CH-012 Day 0 → W03 | ✅ **CLOSED** | 交付的不是 header 而是**判準**（`bootstrap/security.ts:53-78`）：問題不是「這個端點敏感嗎」而是「**這個回應是 entity-scoped 嗎**」—— 約束 8 讓答案永遠是是，故**全域 `no-store, private`，無例外清單**（清單過時的方向是洩漏）。Day 3 API 級驗證：11 個案例全部帶該標頭，含 4xx |
...
| AD-ScopeConcurrency-1 | ~~⭐ **並行範疇汙染是唯一不會拋錯的失敗模式，而它沒有常駐測試**~~ → ✅ **W03 CLOSED** | W02 → W03 | ✅ **CLOSED** | `policy.int.spec.ts:184-201` —— 兩個範疇的 client **交錯** 40 次查詢（交錯而非循序：要排除的是同一條 pooled 連線上範疇外洩，只在請求重疊時出現），**逐列**斷言（汙染的表現是多出來一列）。取代 W02 的 scratchpad 腳本 |
| AD-ScopedClientDI-1 | ~~**「`core-model` 經 DI 取得範疇化 client」仍未被證明**~~ → ✅ **W03 CLOSED，但結論與本條的提議不同** | W02 → W03 | ✅ **CLOSED** | 本條提議的三段拆法（token 在 `api` / 型別在 `core-model` / 實例由 `entity-scope`）**只有後兩段成立**。**token 不存在** —— 範疇化 client 是 per-request，singleton 拿不住、request-scoped 需要憑證來源（M4），今天建就是本條自己警告的「零消費者的 DI token」。實際形狀：型別是 `core-model` 自宣告的**結構型別**，實例走**方法參數**由 `modules` 傳入。⭐ 附帶性質：repository **沒有自己的 client**，所以不存在「忘記範疇化」的路徑。`scope-boundaries.md` §「範疇化 client 如何抵達 core-model」已用量到的形狀取代原設計意圖 |

...
#### Phase W14 — ratio **1.08–1.13 (IN)** ⭐ 兩種量法首次給出**逐位相同**的值

- Bottom-up 4.0 hr → committed **2.0 hr** (mult 0.50) → actual **2.15–2.27 hr**
- **量法是 plan §7 事先宣告的**（含 Day 0，**逐段相加並排除任何 > 60 min 的 commit 間隙** ——
  這正是 `AD-CalibrationWindowCrossSession-1` 在 W13 提出的候選規則）
- **Day 0–3（commit 導出，七個 commit、六個間隙）**：

  | 段 | min |
  |---|---|
  | `7bea684` → `652fb64`（Day 0 verify）| 16.22 |
  | `652fb64` → `88bf634`（建表 + 守衛實戰）| 11.53 |
  | `88bf634` → `a928972`（US-3 裁定）| 19.68 |
  | `a928972` → `d65f535`（polymorphic guard + 契約反轉）| **34.87**（最大）|
  | `d65f535` → `c10be0c`（寫預測）| 3.88 |
  | `c10be0c` → `3d94133`（執行 + 對照 + 還原）| 14.70 |
  | **逐段相加（排除 > 60 min）** | **100.88** |
  | **原始窗口法**（`3d94133` − `7bea684`）| **100.88** |

- **Day 4**：`3d94133` → 本檔寫入 = **28.3 min**（closeout commit 本身再加 1–2 min，**未計入** ⇒ 這是下界）
- **合計 129.2–136 min = 2.15–2.27 hr** ⇒ ratio **1.08–1.13**，**區間兩端同 band**
...
⚠️ **附帶觀察**：W13 的 Day 4 是 28.0 min，W14 是 28.3 min ——
兩個 closeout 的工作量幾乎相同，而它們的 Day 0–3 分別是 91.4 / 100.9 min。
⇒ **closeout 的成本與 phase 的實作量無關**，它是固定開銷。這對 bottom-up 估算有直接意涵：
plan §7 把 closeout 估成 0.5 hr（30 min）**是這七個點裡最準的一項**，
而變異全部來自實作段。

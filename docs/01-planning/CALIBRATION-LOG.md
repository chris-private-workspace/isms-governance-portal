...
**現行乘數**: 0.50 | **資料點**: 10（跨 0.23~1.24；W10 找到分界線，W11 是第一個照那個分界線量的點，**W13 是第一個那條分界線量不出來的點** —— phase 跨夜；**W14 是候選新量法的第一次實測，而它只證明了一半**；**W15 是第一個 plan 預測了方向而方向錯了的點**；**W16 是第一個把前一片的教訓吸收進估算而不是吸收進乘數的點**；**W17 是第一個分子真的被量到而不是被反推的點**）

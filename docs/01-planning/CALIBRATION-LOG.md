...
#### W23 — 2026-08-19 — ratio **0.62 (UNDER)**（第 1 點）

- **Bottom-up** 6.5 hr → **committed** 2.6 hr（mult 0.40）→ **actual ≈ 1.6 hr**（96 min）
- 逐日：Day 0 **15** · Day 1 **16** · Day 2 **24** · Day 3 **12** · Day 4 **~29** min
  （每個 Day 收尾當下記進 `progress.md`，沿用 W22 證明有效的第 3 級提醒）

**這是雜訊還是訊號？—— 訊號，但不在乘數上。**

⛔ **分子有一個必須寫出來的膨脹來源**：Day 4 的 29 分鐘裡**超過一半是計畫外的** ——
(a) 發現 marker 格式漏了 4 種；(b) 發現 E5 會在 closeout 當下誤擋自己。
扣掉這兩件，總計約 79 min，ratio **0.51**。
⇒ **本點相對其他 UNDER 點偏高，不是估得比較準，是做了計畫外的工作。**
寫下這句，是因為下一個人看到 0.62 會推論「0.40 快合適了」，而那個推論是錯的。

⭐ **真正的訊號在另一個欄位**：`actual / bottom-up` = **0.25**，與 W22 的 **0.26** 連成兩點。
本表頭明寫「**低於 0.4 代表你的 bottom-up 估算方式有系統性問題，該修的是估算不是乘數**」。
連續兩個**不同 class、不同性質**的 phase（一個是產品 code、一個零產品 code）都落在 0.25-0.26
⇒ 這不是 class 特性，是**估算方法**。

**根因假設（W22 提出，W23 再次驗證）**：**有藍本的東西被當成沒藍本估**。
本片最清楚的例子是 E5 —— bottom-up 給 **1.5 hr**（按「寫一個新 detector」估），
而 E1–E4 就在同一個檔裡、`run_all` 已註冊、fixture 與測試都有現成慣例可抄。
實際 **24 min**，且那 24 分鐘還包含**兩個我自己的 bug**
（絕對路徑 skip-list、Windows cp1252 解碼）。

**行動**：**KEEP 0.40** —— 單點不 re-point，且本點分子受計畫外工作污染。
⛔ **已開 `AD-BottomUpEstimateInflated-1` 🟡**：`actual/bottom-up` 第 2 次落在 0.25-0.26，
越過「單次離群值」門檻。提議的修法**不是調乘數**，而是估算時先問
「**這個東西有沒有藍本？**」—— 有藍本者按「改既有檔」估，不按「寫新檔」估。

**下次判準**：第 2 個 `docs / audit / template` 點若 ratio 仍 < 0.7 **且**分子未受計畫外工作污染
→ re-point 至 **0.30**；若 `actual/bottom-up` 第 3 次仍 ≈ 0.25
→ **改的是估算流程，不是任何一列乘數**。
...
前三次（W15 / W18 / W21）是「還沒有解法」。**W22 發明了解法並驗證它有效** ——
把提醒從 plan §7 的散文（第 2 級約束，已被實測否證兩次）移到
**checklist 每個 Day 收尾一個具名的 `[ ]`**，四天四筆全部做到。

⇒ W26 復發的根因**不是忘記，是那個解法沒有被寫進 checklist 模板**。
W26 的 checklist 從 frozen template 起草，而 **template 裡沒有那一格** ——
它只活在 W22 那一份 checklist 的實例裡。

⚠️ **這與 `AD-65` 是同一形狀**：「已知的正確方法沒有被套用到第二個場合」。
差別在 `AD-65` 是把方法套到另一個**檔案**，這裡是套到另一個**phase** ——
而後者本來就是 frozen template 存在的理由。
⇒ 提議：`_templates/phase/checklist.md.tpl` 的每個 Day 尾端加一個具名的計時格。
**這是模板變更，需要 2-3 個 phase 驗證後才回流**（`.claude/rules/README.md` 的門檻）。

##### ⭐ re-point 的論證刻意做成「不依賴精確的 actual」

`AD-CalibrationT0PlacementShift-1` 明訂「3-phase 移動平均**不得跨量法計算**」，
而 W22（當日量測）與 W26（事後推估）**正是不同量法** ⇒ 直接把 0.46 與 0.29 並列是有問題的。

**所以 re-point 不建立在 0.29 這個數字上，而建立在一個區間論證**：
推估的 actual 是**下限**（commit 跨距不含思考與閱讀時間）⇒ 真實 actual ≥ 2.75。
**即使真實 actual 翻倍到 5.5 hr，ratio 也只有 0.59 —— 仍然 < 0.7。**
⇒ 判準「第 2 點同 < 0.7」**在任何合理的量法下都觸發**。

⇒ **RE-POINT 0.55 → 0.45 成立**，且它不需要等一個更精確的分子。

##### ⚠️ 另一個序列今天不再單調

`actual / bottom-up` 五點：0.26 → 0.25 → 0.141 → 0.129 → **0.162**（本片）。
`AD-BottomUpEstimateInflated-1` 在 W25 被升為「已驗證」時，引用的是「**四點單調下降**」。
**那個描述從今天起不成立。**

⛔ 但**那條 AD 的論證不依賴單調性** —— 它依賴的是「五個點全部遠低於 1」。
如實記錄這次回升，因為一條被升為「已驗證」的 AD，它引用的證據描述若悄悄失效而沒人記，
下一次有人回頭讀它會以為序列仍在單調下降。

#### Phase W22 — ratio **0.46 (UNDER)** ⭐ 本專案第一個分子完全由當日量測構成的資料點

- Bottom-up **12.5 hr** → committed **6.9 hr** (mult 0.55) → actual **3.18 hr**
- **逐日**（每一筆都在**當日收尾當下**寫進 progress.md，不是事後反推）：
  Day 0 **≈20 min** · Day 1 **≈35 min**（`15:42 → 16:17`）· Day 2 **≈36 min**（`20:52 → 21:28`）·
  Day 3 **≈75 min** · Day 4 **≈25 min**（`09:31 → closeout commit`）
- **量法**（寫到能重現）：T0 = 該日第一個動作，T1 = 該日最後一個 commit 或收尾動作。
  ⚠️ **plan / checklist 起草不計入分子** —— 它發生在 `e17464e`，在建分支之前；
  而 bottom-up 的 12.5 hr **也沒有**包含它。分子分母在這一點上一致。
  ⚠️ Day 1 commit 16:20 → Day 2 開工 20:52 的 4.5 小時是**等待，不是工時**，未計入。

- **發生了什麼**：⭐ **這一點最重要的產出不是 ratio，是它終於是一個可信的 ratio。**
  `AD-CalibrationNoTimeRecord-1` 已三次（W15 / W18 / W21），三次都是事後由 commit
  author date 反推 —— 反推得到的是**下限**，不是量測。W22 把提醒從 plan §7 的散文
  （第 2 級約束，**已被實測否證兩次**）移到 **checklist 每個 Day 收尾一個具名的 `[]`**，
  四天四筆全部做到了。**差別不在態度，在於提醒放在會被勾的清單上而不是會被讀一次的散文裡。**

- **⚠️ 真正的訊號在另一個比值**：`actual / bottom-up` = **0.26**，
  遠低於 `CALIBRATION-MATRIX.md` 明訂的 **0.4 下限**。那份文件對這個情況的判決很明確：
  **「低於 0.4 代表你的 bottom-up 估算方式有系統性問題，該修的是估算不是乘數」**。
  W07 出現過同一個訊號（0.17）而當時沒有後續動作。

  高估點指得出來，而且有共同形狀 —— **有藍本的東西被當成沒有藍本估**：

  | bottom-up 項 | 估 | 實際 | 有沒有藍本 |
  |---|---|---|---|
  | 端點 + 測試 | 1.5 hr | Day 1 合計 35 min（含 seed）| ✅ `policy.controller.ts:89-102` 逐字可抄 |
  | seed | 2 hr | ↑ 同上 | ⚠️ 半個 —— `int-global-setup.js` 是形狀參考 |
  | fetch 層 + 兩頁 | 3 hr | Day 2 合計 36 min | ✅ `app/page.tsx:45` 的 fetch pattern |
  | 404 與測試 | 1 hr | 併入 Day 2 | ✅ W19 已有 not-found 卡 |
  | **drive-through** | **2 hr** | **75 min** | ❌ **沒有藍本，只能真的做** |
  | closeout | 2 hr | Day 4 ≈25 min | ✅ 模板固定 |

  ⭐ **唯一估得準的是唯一沒有藍本的那一項。** 這正好解釋了為什麼 plan §7 當初
  **刻意不用 `pattern-reuse-feature` 0.50** —— 它說「後端那一半確實是複製，而前端那一半沒有藍本」。
  那個判斷方向對了，**幅度錯了**：前端那一半的「沒有藍本」只是「本 repo 沒有產品畫面呼叫過 API」，
  而 fetch 本身、狀態機、i18n、測試形狀**全部都有**。真正沒有藍本的只有 drive-through。

- **是雜訊還是訊號**：**訊號，但落點不在乘數。** 乘數修的是「bottom-up 系統性高估約 2 倍」；
  這裡的偏差是 **3.6 倍**，超出乘數能吸收的範圍，而且來源是**拆解時沒有問「這一項有沒有藍本」**。

- **行動**: **KEEP 0.55**（單點不 re-point，3-phase 移動窗口）。
  **下一個 `greenfield-feature` 的 plan §7 必須逐項標註「有藍本 / 無藍本」**，
  無藍本的項才用完整的 bottom-up 估算 —— 若第 2 點仍 < 0.7，
  **改的是拆解方式不是乘數**（0.45 只是把同一個估算誤差換一個地方藏）。
...
- **逐段**：Day 0–3 **~3.15 hr**（`a401215` 2026-08-18T08:54:13+08:00 →
  `7da314f` 12:03:08，**由 author date 反推**）· Day 4 **~1.3 hr**（本 session 實測，
  五段裡**唯一真的量到的一段**）
...
**現行乘數**: **0.45**（W24 re-point，本欄第一次）| **資料點**: 12（跨 0.23~1.24；
**W24 是第一個觸發 re-point 的點，而它同時證明 re-point 不夠** ——
真正被高估的是 bottom-up 本身，見該則；**W18 是第一個「乾淨的 UNDER」——
...
#### Phase W24 — ratio **0.377 (UNDER)** ⭐ 第 12 點，同量法 ⇒ **判準字面觸發，re-point 0.45**

- Bottom-up **20.5 hr** → committed **7.7 hr**（mult 0.50 × agent_factor 0.75）→ actual **~2.9 hr**
- **逐段**（wall-clock）：Day 0+1 **46 min**（T0 `2026-08-19T07:44:24Z`）· Day 2 **45 min** ·
  Day 3 **37 min** · Day 4 **~45 min**（本檔與導航檔更新）
- **量法與 W18 相同** —— T0 蓋在讀第一個檔案前，分子完整（含背景等待）。
  ⇒ W18 訂的「第 12 點**同量法**再 <0.7 則 re-point 0.45」**字面觸發**，照做。
- **發生了什麼**：本片是 W22 risks 垂直切片的 **slice 2/N**，藍本逐項複用 ——
  `lib/api/*` 的信封與錯誤型別、`NoSource` 標記、三狀態、seed 的 upsert 形狀。
  ⇒ 「pattern-reuse」在本片是**字面成立**的，而 bottom-up 沒有把「藍本已存在」折進去。
- **⛔ re-point 治標不治本，這句要寫在行動之前**：0.45 只把 committed 從 7.7 降到 6.9，
  ratio 仍是 **0.42** —— 一樣在 band 外。真正的訊號是 `actual / bottom-up`：
  W22 **0.26** · W23 **0.25** · W24 **0.141**，三點都遠低於本表表頭寫的 0.4 乘數下限。
  ⇒ **被系統性高估的是 bottom-up 本身，不是某個 class 的乘數。**
- ⭐ **B1 vs B2 並列（`AD-BottomUpEstimateInflated-1` 第 3 個資料點）**：
  B2（AI 步驟估）7.6 hr → `actual/B2` **0.382**；calibrated B1 7.7 hr → **0.377**。
  ⚠️ plan §7 **預先**警告本片分辨力低（兩者幾乎重合），結果確實只差 **0.005**。
  ⛔ 正確讀法是**兩者都高估約 2.6 倍**，不是「兩種方法都對」——
  B2 唯一勝出的是它省掉了乘數這一層中介。要分辨仍需一個 B1/B2 分岔較大的 phase。
- **是雜訊還是訊號**：訊號。W18 (0.545) → W24 (0.377) 是**同量法的兩個連續點**，方向一致。
- **行動**：**RE-POINT 0.50 → 0.45**。⚠️ **若第 13 點同量法仍 <0.7，不要再調乘數** ——
  那時該動的是 bottom-up 的產生方式；繼續調乘數只是把同一個誤差搬家。
...
- **逐段**（commit author date 為段界）：T0 `2026-08-17T02:47:33Z` →
  `3f69e08` **45.60 min** · `0dc410a` **25.08** · `4837316` **1.18** · `d8179af` **19.95** ·
  `3ec5f0b` **6.18**。⚠️ 段 1 超過 30 min，依宣告的量法必須逐條說明：
  它**不是等待間隙**，是單一段連續工作（Day-0 三-prong：讀 on-demand 規則、派 2 個 agent、
  讀 5 份規格段落、跑 3 組 baseline、寫 11 條 drift、建 checklist）⇒ **計入**。
- ⚠️ **未計入最後一段**（本檔與 matrix 回填、導航檔更新，發生在 `3ec5f0b` 之後，估 ~20 min）。
...
**現行乘數**: 0.65 | **資料點**: 7（**4 個同單位可用** —— W04 0.81 / W07 0.30 / **W12 1.025** / **W25 0.199**）

#### Phase W25 — ratio **0.199（UNDER band）** ⛔ 登記在案的預測往下沒中

- Bottom-up **20.5 hr** → committed **13.3 hr**（mult 0.65）→ actual **≈2.64 hr**
- **窗口**：前一片的 post-merge commit `664fcdc` **09:01:29** → Day-4 最終 gate sweep **11:40:08**（**不含 commit / PR**）。
  ⚠️ **是牆鐘上界不是分項加總** —— progress.md 未逐日記時，所以真實工時 ≤ 2.64 hr，
  ratio 因此是**保守（偏高）**的。`AD-CalibrationNoTimeRecord-1` 的第 4 次出現。
- ⛔ **plan §7 登記了一個可證偽的預測，而它沒中**：預測 actual 落在 **2.9–5.3 hr**
  （由 W22 0.26 / W23 0.25 / W24 0.141 × bottom-up 20.5 推得）⇒ ratio 0.22–0.40。
  實測 **2.64 hr / 0.199**，**低於區間下限**。
  ⭐ **預測沒中比預測中了更有價值**：它證明那三個歷史比例本身仍在下降，不是穩定在 0.14 附近。
- ⭐⭐ **`actual / bottom-up` = 0.129，第 4 個資料點，且四點單調下降**：

  | Phase | class | actual/bottom-up |
  |---|---|---|
  | W22 | `greenfield-feature` | 0.26 |
  | W23 | `docs / audit / template` | 0.25 |
  | W24 | `pattern-reuse-feature` | 0.141 |
  | **W25** | **`spike`** | **0.129** |

  **跨四個不同的 scope class 單調下降** ⇒ 這不是某個 class 的乘數問題，
  是 **bottom-up 的產生方式本身**沒有校準基礎。
  ⇒ `AD-BottomUpEstimateInflated-1` 由「候選」升為**已驗證（4/3）**。
- ⛔ **不 re-point，而且理由要寫清楚**：把 `spike` 從 0.65 調到 ~0.13 會讓 committed 變成 2.7 hr，
  數字上「準」了 —— 但**下一片的 bottom-up 仍然會虛胖**，然後在第 8 個點再調一次。
  **乘數在追一個被污染的分子。** 該換的是分子的產生方式（提議見 retro Q6）。
- **與 W12 的對照**：W12 的 `actual/bottom-up` 0.667 曾被記為「有條件的正向點」，
  理由是它的 bottom-up 是 **Day-0 之後重估**的。W25 的 bottom-up **也在 Day-0 之後修訂過**
  （18.5 → 20.5），⇒ 同樣享有那個「事後比較準」的優勢，**卻仍然只有 0.129**。
  這讓 W12 那個點更像是離群值而非趨勢反轉。
- **行動**: **KEEP 0.65**；AD 升級為已驗證，估算方法的變更提案進 BACKLOG。

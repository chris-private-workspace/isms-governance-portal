...
**現行乘數**: 0.50 | **資料點**: 5（跨 0.23~1.13；W10 找到分界線，**W11 是第一個照那個分界線量的點**）

#### Phase W11 — ratio **1.13 (IN)** ⭐ 本欄第一個 IN-band 點，也是第一個事先宣告量法的點

- Bottom-up 3.0 hr → committed 1.5 hr (mult 0.50) → actual **~1.7 hr**
  （`52465f0` 10:21 → closeout commit ~12:00，**含 Day 0**，單日無跨夜間隙）
...
- **是雜訊還是訊號**：⛔ **一個點什麼都不是**。真正的判準是「連續 3 個**同一種**量法的點」，
  而今天符合條件的點數是 **1**。0.23 / 0.24 / 0.50 / 0.84 四個舊點的量法仍不可考。

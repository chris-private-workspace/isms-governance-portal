...
#### Phase W05 — ratio 0.71（IN band，**僅在修正定義下**）⛔ 這一則的重點是定義破了

- Bottom-up 14 hr → committed 7.0 hr (mult 0.50) → actual **~5.0 hr（修正）/ ~15.3 hr（字面）**
- **⛔ 預先宣告的定義在這裡失效，而失效方式是可預測的**：plan §7 宣告
  ...
  字面套用得到 15.3 hr / ratio **2.19**；扣除間隙是 5.0 hr / ratio **0.71**。
...
- **行動**：**KEEP 0.50**，本點標記為「定義受污染」→ `AD-CalibrationMetric-2`。
  在定義修好之前，`pattern-reuse-feature` 的有效樣本數是 **0**，不是 1。

...
| 10 | **`AD-CountBeforeLastEdit-1` 的機械導出** —— `BACKLOG.md` §Open 的總數與三個優先度計數改由腳本重算並比對，不符即 fail | ⬜ | **排 W09 Day 0**（不另開 CH）—— 使用者 2026-08-13 裁決「**保留總數 + 機械導出**」 | ⭐ **完全複製已成功的先例**：`AD-EntityCountDerivation-1` / `AD-EntityIndexIncomplete-1` 是同一個形狀（手寫計數器），排進 W08 Day 0 由 `check_entity_index.py` 一次關掉，**沒有吃掉 `CH-017` 的治理配額**。⚠️ **今天只做了一半** —— 數字改成「先跑指令再抄」，但沒有任何東西會在不符時 fail，而本條的證據就是「靠紀律重跑」會在最警覺時失效（W08 closeout 認真做了兩種數法對照，然後又加了一條而沒重數）|

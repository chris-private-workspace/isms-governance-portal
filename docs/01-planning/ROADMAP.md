...
| 9b | ⭐⭐ **重新檢視 `required_linear_history`** —— 這條 AD 自己設的門檻被靜默跨過兩次 | ✅ | — | **W21（2026-08-18）—— 裁決：維持 `rebase`，設定不動。** ⭐ **理由不是慣性，而且它一直沒被寫下來過**：一條直線比 DAG 容易對**稽核人員**證明「什麼時候發生了什麼」，而本專案是一套要通過自己強制的控制項的 ISMS 平台（guardrail 1 / 2）。三選一的取捨已攤開：rebase（線性 ✅ / SHA 穩定 ❌ **已付 11 次** / 逐 commit 敘事 ✅）· squash（✅ / ❌ / ❌ **摧毀本專案投入最多的 commit message 資產**）· merge commit（❌ / ✅ / ✅）。**接受 SHA 改寫的代價**，換取線性歷史的可稽核性。⭐⭐ **配套（這才是這一列真正產出的東西）**：**凡是需要跨 merge 存活的引用，錨點是 author date + subject，SHA 只當索引** —— W14 已量到 author date 逐秒不受 rebase 影響（第 4 次確認），而 W15 的 calibration 完全由它導出、**數字撐過了 rebase 即使 SHA 沒有**。⇒ `check_sha_anchors.py` 繼續重指 SHA（為了可點擊），但**證明力不再掛在 SHA 上**。⚠️ 這一列被漏掉 **3 次**（W17 第 9 · W18 第 10 · W20 本要接手而全片回退），第 4 次沒有發生的唯一原因是它被寫進了 W21 checklist §4.2 的一個具名 `[ ]` —— **第 4d 列的警語再次成立：ROADMAP 的列不是 checklist 的列**。原備註保留於下 —— ⛔ **本列問的是那個設定，不是第 9 列的 detector** —— `AD-DesignNoteAnchor-1` 原文寫「若走到第 9 / 10 次，該被重新檢視的是**那個設定**，不是又一版錨點規則」。實際序列 W15=7 · W16=8 · **W17=9** · **W18=10** · W19=11 ⇒ **第 9 與第 10 都發生了而重審一次都沒觸發**，因為 W17 / W18 的 retro 都沒記序數，計數只活在 BACKLOG 一格散文裡而**沒有任何東西會在 merge 時遞增它**。⭐ **關鍵區分**：`check_sha_anchors.py` 每一次都正常運作、每一次都抓到也都修好了 —— 失效的是**它上面那一層**（「發生幾次、到第幾次該重新決策」無機械承載）⇒ 與 W19 的 25 個死控件同形：**守衛在、條件成立、沒有人在看**。**要裁決的取捨**（三選一，細節在 BACKLOG 該 AD）：rebase（線性 ✅ / SHA 穩定 ❌ 已付 11 次 / 逐 commit 敘事 ✅）· squash（✅ / ❌ / ❌ 摧毀本專案投入最多的 commit message 資產）· merge commit（❌ / **✅** / ✅）。⚠️ **不預設答案是「改掉」** —— 線性歷史對一套 ISMS 平台有非顯而易見的加分：一條直線比 DAG 容易對稽核人員證明「什麼時候發生了什麼」，那可能正是當初的真實理由，只是沒被寫下來。⛔⭐ **依第 4d 列已證實的限制，本列進了 ROADMAP 不等於它會被做** —— 它必須在下一片的 `checklist.md` 上有一個具名的 `[ ]`（`AD-CountBeforeLastEdit-1` 量到「ROADMAP 的列不是 checklist 的列」，兩次 closeout 都沒東西發現它沒被做）。⚠️ 若下一片的 plan 起草時本列沒有被搬進 checklist，**那就是同一個失效的第三次演出** | [`BACKLOG.md`](./BACKLOG.md) `AD-DesignNoteAnchor-1` |

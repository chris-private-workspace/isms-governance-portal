...
- **shell fragment（`02-app-shell`）未納入本表** —— 它是 W19 第一個移植物、規則來源本身。
- **`30-ai-drawer` 已於 Day 4 移植**（`components/shell/AiDrawer.tsx`，掛在 shell 上所以 27 頁都有），
  但**未納入本表的逐字比對** —— 它是覆蓋在畫面之上的元件，沒有對應的 `page.tsx` 可以並排。
  已做 drive-through：啟動鈕 fixed / z-60、面板 392px、4 個建議提示可用、
  **自由輸入不作答只給誠實通知**、**零廠商字串洩漏**。

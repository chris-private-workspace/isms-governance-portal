...
而那個條件早已不成立（**20** 張表、172 個 int 測試、真 PostgreSQL），**沒有任何東西在等它失效**。
<!-- 19 → 20，審計 #5 AD-18（2026-08-15 修正）。同一次 closeout 已經更正過一次這個錯的表數
     （R4 的 18 → 21），但那次 grep 的是「覆蓋率片語」而不是**那個錯的值**，所以這句背景敘述逃掉了。
     ⭐ 教訓：一次更正的掃描 pattern 必須是「錯的值」，不是「它出現的句型」。
     對照組：同一天寫的 `14-adr/0003-audit-trail-hash-chain.md:14`（"twenty tables"）與
     W12 `plan.md:62`（「20 張表」）都是對的 —— 不是全錯，是兩份文件錯。
     ⛔ `CH-029:13` 的 "nineteen business tables" **刻意不改**：已 merge 的 change record 是歷史
     快照，本專案不回頭改其內文（`CH-022:190` 先例）。design note 是**活參考**，所以這裡改。 -->
...
**actual 4.00 hr**（`7503f8d` 14:33:25 → `f7a0c03` 18:33:23，author date）⇒ ratio **1.025 IN**
